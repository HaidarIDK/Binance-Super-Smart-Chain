#!/usr/bin/env bash
#
# Run a minimal BSSC cluster.  Ctrl-C to exit.
#
# Before running this script ensure standard BSSC programs are available
...
ok=true
for program in bssc-{faucet,genesis,keygen,validator}; do
  $program -V || ok=false
done
$ok || {
  echo
...
export RUST_LOG=${RUST_LOG:-bssc=info,bssc_runtime::message_processor=debug} # if RUST_LOG is unset, default to info
export RUST_BACKTRACE=1
dataDir=$PWD/config/"$(basename "$0" .sh)"
ledgerDir=$PWD/config/ledger

BSSC_RUN_SH_CLUSTER_TYPE=${BSSC_RUN_SH_CLUSTER_TYPE:-development}

set -x
if ! bssc address; then
  echo Generating default keypair
  bssc-keygen new --no-passphrase
fi
validator_identity="$dataDir/validator-identity.json"
if [[ -e $validator_identity ]]; then
  echo "Use existing validator keypair"
else
  bssc-keygen new --no-passphrase -so "$validator_identity"
fi
...
if [[ -e $validator_vote_account ]]; then
  ...
  bssc-keygen new --no-passphrase -so "$validator_vote_account"
fi
...
if [[ -e $validator_stake_account ]]; then
  ...
  bssc-keygen new --no-passphrase -so "$validator_stake_account"
fi
